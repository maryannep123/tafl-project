const generateRightmostDerivation = (grammar, startSymbol, target) => {
...
const dfsDerivation = (grammar, startSymbol, target) => {
  ...
  const recurse = (symbol, steps) => {
    ...
    const rightmost = findRightmostNonTerminal(symbol, grammar);
    if (!rightmost) return null;

    const { index, nonTerminal } = rightmost;
...
const applyRecursiveNTimes = (grammar, symbol, production, n, nonTerminal) => {
  ...
  for (let i = 0; i < n; i++) {
    const index = current.lastIndexOf(nonTerminal);
...
const applyEpsilon = (grammar, symbol, nonTerminal, existingSteps) => {
  const index = symbol.lastIndexOf(nonTerminal);
  if (index === -1) return null;
...
const bfsDerivation = (grammar, startSymbol, target, maxDepth = 15) => {
  ...
  while (queue.length > 0) {
    ...
    const rightmostIndex = findRightmostNonTerminal(symbol, grammar);
    if (!rightmostIndex) continue;
    
    const { index, nonTerminal } = rightmostIndex;
...
const findRightmostNonTerminal = (symbol, grammar) => {
  for (let i = symbol.length; i >= 0; i--) {
    for (const nonTerminal of Object.keys(grammar).sort((a, b) => b.length - a.length)) {
      if (symbol.substring(i - nonTerminal.length, i) === nonTerminal) {
        return { index: i - nonTerminal.length, nonTerminal };
      }
    }
  }
  return null;
...
module.exports = { generateRightmostDerivation };